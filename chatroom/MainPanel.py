...
class MainPanel:
    ...
    def show(self):
        global main_frame
        main_frame = Tk()
        main_frame.title(f"python聊天室-{self.username}")
        ...
        Label(main_frame, text="在线用户", font=("宋体", 13), bg="#333333",
        ...
        self.send_text.grid(row=2, column=1, sticky=W + E + N + S, padx=(10, 30), pady=5)
        send_sc_bar["command"] = self.send_text.yview
        self.send_text["yscrollcommand"] = send_sc_bar.set
        Label(main_frame, text="shift+enter可以直接发送消息", font=("宋体", 13), bg="#333333",
               fg="white").grid(row=3, column=1, padx=10, sticky=W)
        Button(main_frame, text="发送", bg="blue", font=("宋体", 14), fg="white", command=self.send_func) \
            .grid(row=3, column=1,sticky=E,padx=(10,30),pady=10)
        # Button(main_frame, text="清空", bg="red", font=("宋体", 14), fg="white", command=self.clear_send_text) \
        #     .grid(row=3, column=1, pady=5, sticky=E, padx=(110, 0), ipady=3, ipadx=10)
        self.main_frame = main_frame
        self.recv_message("系统消息", "欢迎用户{0}加入聊天室！".format(self.username))
        keyboard.add_hotkey('shift+enter', self.send_func, args=())
